...
def calc_miou_tf(preds, targets):
...
def calc_map_iou_range_tf(preds, targets, iou_thresholds=np.arange(0.5, 1.0, 0.05)):
...
def evaluate_picosam_tf(model, loader, name):
    ...
    for i, (x, y, _, _) in enumerate(tqdm(loader)):
        ...
        preds.append(pred)
        gts.append(y_tf)
        mious.append(calc_miou_tf(pred, y_tf))
    ...
    print(
        ...
        f"mAP@[0.5:0.95]: {calc_map_iou_range_tf(preds, gts):.4f}"
    )
...
if __name__ == "__main__":
    ...
    # OR
    tf_model = TFLiteModelWrapper("PicoSAM2_student.tflite") # Pass the file
    tf_model_quant = TFLiteModelWrapper("PicoSAM2_student_int8.tflite") # Pass the file
    ...
    #evaluate_picosam_tflite_int8(tf_model_quant, coco_loader, "PicoSAM2 TensorFlow Quantized (COCO)")
