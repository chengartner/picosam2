...
def train():
    ...
    # New: Next 5 lines make sure only 10% of dataset is used
    subset_len = int(len(dataset) * 0.1)  # use 10% of dataset
    train_len = int(subset_len * 0.95)
    val_len = subset_len - train_len
    subset_ds, _ = random_split(dataset, [subset_len, len(dataset) - subset_len])
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f"PicoSAM2_epoch{epoch + 1}_subset.pt"))
